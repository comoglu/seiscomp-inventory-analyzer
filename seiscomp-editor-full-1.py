...
class SeisCompInventoryEditor(QMainWindow):
    ...
    def update_stream(self):
        if hasattr(self, 'current_element'):
            try:
                ...
                self._update_element_text(self.current_element, 'sampleRate', self.stream_sampleRate.text())
    ...
    def populate_stream_fields(self, stream):
            ...
            self.stream_sampleRate.setText(
                str(float(self._get_element_text(stream, 'sampleRateNumerator', '0')) / 
                    float(self._get_element_text(stream, 'sampleRateDenominator', '1')))
            )
